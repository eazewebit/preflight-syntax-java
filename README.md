...
## Retrieving Modified File Content

The library provides convenient methods to retrieve the modified file content
after validation, without writing to disk. This is useful when you need to:

- Write the modified content to a different location
- Pass the content to other tools for further processing
- Generate diffs between original and modified content
- Apply the modification only after all validations pass
...
// Option 1: Get modified content as a single string
String modifiedContent = library.getModifiedContent(request);
System.out.println(modifiedContent);

// Option 2: Get modified content as a list of lines
List<String> modifiedLines = library.getModifiedLines(request);
modifiedLines.forEach(System.out::println);

// Option 3: Validate first, then retrieve content
ValidationResult result = library.validate(request);
if (result.isValid()) {
    String safeContent = library.getModifiedContent(request);
    // Write to file or pass to another tool
...
| `BatchModificationRequest` | Immutable descriptor for multiple line-range replacements. Use `builder()` to construct. |
...
| Method | Description |
|--------|-------------|
| `validate(ModificationRequest)` | Validates a single modification and returns a `ValidationResult`. |
| `validate(BatchModificationRequest)` | Validates multiple modifications and returns a `ValidationResult`. |
| `validateAllLanguage(request)` | Auto-selects validation strategy based on file language. |
| `getModifiedContent(request)` | Returns modified content as a `String` without writing to disk. |
| `getModifiedLines(request)` | Returns modified content as a `List<String>` without writing to disk. |
| `validateSource(language, source)` | Validates raw source code directly. |
| `invalidateCache(path)` | Evicts cached content for a specific file. |
| `clearCache()` | Evicts all cached file content. |
